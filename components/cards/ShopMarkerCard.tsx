...
type ShopMarkerCardProps = {
  shopData: ShopData;
  onPressFn?: () => void | undefined;
  extraClasses?: string;
};

export default function ShopMarkerCard(
  props: ShopMarkerCardProps,
): JSX.Element {
  return (
    <TouchableOpacity onPress={props.onPressFn}>
      <View
        className={`${props.extraClasses} rounded-lg p-2 bg-lightbg dark:bg-darkbg`}
      >
        <View className="flex flex-row">
          ...
          <View className="w-auto" style={{ paddingLeft: 10 }}>
            ...
            <BadgeSecondary extraClasses="px-1 text-wrap">
              {`${props.shopData?.stocks.length} produit${props.shopData?.stocks.length > 1 ? "s" : ""} chez ce producteur `}
            </BadgeSecondary>
          </View>
        </View>
      </View>
