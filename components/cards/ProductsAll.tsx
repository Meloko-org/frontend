...
type CardProductsAllProps = {
	productData: ProductData
	onPressFn?: ((event: GestureResponderEvent) => void) | undefined
	extraClasses?: string
}

export default function CardProductsAll(props: CardProductsAllProps): JSX.Element {
		...
		<View className={`${props.extraClasses} rounded-lg shadow-sm bg-white p-2 dark:bg-darkbg flex flex-row w-full`}>
			<View className="flex flex-row items-center w-full">
				<View className="flex flex-row items-center rounded-lg w-auto h-full">
					<Image source={require('../../assets/images/tomate.webp')} className="rounded-full w-20 h-20" alt={`logo de la boutique ${props.productData.name}`} resizeMode="cover" width={96} height={64}/>
				</View>
				<View className="h-full px-2 items-start w-3/5">
					<TextHeading4>{ `${props.productData.family.name} ${props.productData.name}` }</TextHeading4>
						<TextBody1>{ props.productData.description }</TextBody1>
						<PricePer>{props.productData.price.$numberDecimal}€ / 100gr</PricePer>
				</View>
